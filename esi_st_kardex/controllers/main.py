...
class KardexDownloadController(http.Controller):
    ...
    @http.route('/esi_st_kardex/excel/<int:report_id>', type='http', auth='user', methods=['GET'])
    ...
    @http.route('/esi_st_kardex/pdf/<int:report_id>', type='http', auth='user', methods=['GET'])
    def download_pdf(self, report_id, **kwargs):
        report = self._get_report(report_id)
        if not report:
            return request.not_found()

        action = request.env.ref('esi_st_kardex.kardex_product_report_pdf')
